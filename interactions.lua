...
function render_chat(screen_w, screen_h, delta_time)
	...
	-- make chat messages show up for longer (default is 5 seconds with 0.5 secodn fade)
	if g_chat.is_chat_box then
		factor = 1
	elseif #messages > 0 then
		display_time = 10000
		display_time_fade = 500
		local time_since_last_message = update_get_time_ms() - messages[1].timestamp
		factor = invlerp_clamp(time_since_last_message, display_time+display_time_fade, display_time) ^ 2
		--print(update_get_time_ms(), messages[1].timestamp, time_since_last_message, factor)
	end
	...
	-- re-ordered to allow filting out of server notifications
	show_server_notifications = 0
	if #messages > 0 then
		-- draw chat backgrounud
		update_ui_rectangle(border, border, chat_w, chat_h, color8(10, 10, 10, math.min(128,math.floor(factor * 255))))
		for i = 1, #messages do
			...
			if msg.type == e_chat_message_type.server_notification then
				...
				if show_server_notifications == 0 then
					break
				end
			elseif msg.type == e_chat_message_type.player_team then
				sender_col = g_color_text_team
				--print(msg.type)
				--col = update_get_team_color(e_chat_message_type.player_team)
				--sender_col = update_get_team_color(e_chat_message_type.player_team)
			end
